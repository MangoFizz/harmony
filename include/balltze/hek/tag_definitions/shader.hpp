...
#ifndef BALLTZE_API__HEK__TAG_DEFINITION__SHADER_HPP
#define BALLTZE_API__HEK__TAG_DEFINITION__SHADER_HPP
...
namespace Balltze::HEK::TagDefinitions { 
	...
	struct Shader {
		Memory::BigEndian<ShaderFlags> shader_flags;
		Memory::BigEndian<ShaderDetailLevel> detail_level;
		Memory::BigEndian<float> power;
		ColorRGB color_of_emitted_light;
		ColorRGB tint_color;
		Memory::BigEndian<ShaderPhysicsFlags> physics_flags;
		Memory::BigEndian<MaterialType> material_type;
		Memory::BigEndian<std::uint16_t> shader_type;
		PADDING(2);
	};
