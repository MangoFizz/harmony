...
#ifndef BALLTZE_API__HEK__TAG_DEFINITION__SHADER_TRANSPARENT_WATER_HPP
#define BALLTZE_API__HEK__TAG_DEFINITION__SHADER_TRANSPARENT_WATER_HPP
...
namespace Balltze::HEK::TagDefinitions { 
	...
	struct ShaderTransparentWaterRipple {
		...
		Memory::BigEndian<float> animation_velocity;
		Vector2D map_offset;
		Memory::BigEndian<std::uint16_t> map_repeats;
		Index map_index;
		PADDING(16);
	};
	...
	struct ShaderTransparentWater : public Shader {
		Memory::BigEndian<ShaderTransparentWaterFlags> water_flags;
		PADDING(2);
		PADDING(32);
		...
		Memory::BigEndian<float> ripple_animation_velocity;
		Memory::BigEndian<float> ripple_scale;
		TagDependency ripple_maps;
		Memory::BigEndian<std::uint16_t> ripple_mipmap_levels;
		PADDING(2);
